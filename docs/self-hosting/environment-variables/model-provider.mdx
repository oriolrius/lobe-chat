...
- Type: Required
- Description: This is the API token you obtained from Replicate. You can get it from [Replicate API tokens page](https://replicate.com/account/api-tokens).
- Default: -
- Example: `r8_xxxxxx...xxxxxx`

### `REPLICATE_PROXY_URL`

- Type: Optional
- Description: If you manually configure the Replicate API proxy, you can use this configuration item to override the default Replicate API request base URL.
- Default: `https://api.replicate.com/v1`
- Example: `https://my-replicate-proxy.com/v1`

### `REPLICATE_MODEL_LIST`

- Type: Optional
- Description: Used to control the Replicate model list. Use `+` to add a model, `-` to hide a model, and `model_name=display_name` to customize the display name of a model. Separate multiple entries with commas. The definition syntax follows the same rules as other providers' model lists.
- Default: `-`
- Example: `-all,+black-forest-labs/flux-1.1-pro,+black-forest-labs/flux-2-pro=FLUX 2 Pro`
